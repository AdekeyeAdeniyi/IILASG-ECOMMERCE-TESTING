    ...
    <aside
      ...
      <div className="px-2 py-4">
        <button
          ...
          {!collapsed && <p className="text-sm sm:text-base transition-all duration-300">Collapse</p>}
        </button>
...
const SidebarItem = ({ icon, label, to, collapsed }: SidebarItemProps) => {
  ...
  const styledIcon = React.isValidElement(icon)
    ? React.cloneElement(icon as React.ReactElement<any>, {
        className: isActive ? "text-primary-500" : "text-grey-900 transition-colors duration-300 group-hover:text-primary-500",
      })
    : icon;
  ...
  return (
    <Link
      ...
      className={`
        ...
        ${isActive ? "bg-primary-50 border-primary-500 text-primary-500" : "border-white text-grey-900 hover:text-primary-500 hover:bg-primary-50 hover:border-primary-500"}
      `}>
      <span className="flex-shrink-0 transition-colors duration-300">{styledIcon}</span>

      {!collapsed && (
        <span className={`whitespace-nowrap text-sm sm:text-base font-normal transition-colors duration-300 ${isActive ? "text-primary-500" : "group-hover:text-primary-500"}`}>{label}</span>
      )}
    </Link>
  );
};
